#!/usr/bin/env python3
"""Upload the finished Qwen 3.5-4B run directory to Hugging Face."""
...
def model_card(repo_id: str) -> str:
    ...
    qat = model.LORA["qat_scheme"]
    qat_text = f"QAT `{qat}`" if qat else "no QAT"
    return f"""---
license: apache-2.0
...
This derivative is subject to the [Apache 2.0](https://www.apache.org/licenses/LICENSE-2.0) license of the Qwen base model.
...
LoRA: RsLoRA r={model.LORA["r"]}, alpha={model.LORA["lora_alpha"]}, {qat_text}.
